...
- **Interrupted run** — a run whose process ended (a `kill -9` or a crash)
  ...
  the run's event stream `run-interrupted`, distinct from `FAILED`, which a
  graceful signal handler still writes on its own.
...
- **Stage awaiting judgment** — the stage an **interrupted run** was in after
  its session finished and before its judging completed. Its
  `<run>.<stage>.json` carries `AWAITING_STAGE_JUDGE`, the stage name, the
  Judge's input and the model, effort and budget the stage ran with. Judging
  ending overwrites that file either way, with a scorecard or with a stop
  record, so a file still carrying the status is one whose run died in that
  window. It writes no checkpoint, so it has neither a lineage nor a stop
  reason, it declares no corpus files, and the parsed exchanges its input holds
  are not the raw transcript. It is not a **stopped stage**: nothing judged it
  and nothing failed.
...
- **Group median grade**: the grade a confirmation group's reps received at
  ...
  and no Judge gives them a letter, though its row still counts them. Rule
  accepted unattended as unsettled, pending the operator's confirmation (doc-152, decision 3).

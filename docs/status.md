...
| Local read UI and event API    | Partial browser views, server-side event streaming, a per-run record API with each stage's cost, tokens, artifacts in and out, and the final outcome, and run history rows with step grades, task grade, cost and wall time, naming each figure no record holds                                                                                                                                                                                                                             | [Router](../client/src/router.tsx), [API](../src/server/api.ts), [Run record](../src/server/run-record.ts), [Run history](../src/server/run-history.ts)                                                                                                                          |
